...
function clickSubMenu1(element) {
	...
	$(element).children().attr("class","on");
	// 按指定地址加载主页面(iframe)
	$("#mainPage").attr("src",$('input[name=basepath]').val()+"/getBsList");
}
...
function clickSubMenu2(element) {
	...
	$(element).children().attr("class","on");
	// 按指定地址加载主页面(iframe)
	$("#mainPage").attr("src",$('input[name=basepath]').val()+"/getCoList");
}
...
function clickSubMenu3(element) {
	...
	$(element).children().attr("class","on");
	// 按指定地址加载主页面(iframe)
	$("#mainPage").attr("src",$('input[name=basepath]').val()+"/getOrList");
}
...
function clickSubMenu4(element) {
	...
	$(element).children().attr("class","on");
	// 按指定地址加载主页面(iframe)
	$("#mainPage").attr("src",$('input[name=basepath]').val()+"/reportPage");
}
